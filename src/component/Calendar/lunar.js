const calendar = {
  ...
  getTerm(y, n) {
    ...
    const _info = [0, 1, 2, 3, 4, 5].map(item =>
      parseInt("0x" + _table.substr(item * 5, 5)).toString()
    );
    const _calday = [
      _info[0].substr(0, 1),
      _info[0].substr(1, 2),
  ...
  toChinaDay(d) {
    ...
    return (
      dayMap[d] || calendar.nStr2[Math.floor(d / 10)] + calendar.nStr1[d % 10]
    );
  },
  ...
  solar2lunar(y, m, d) {
    ...
    //未传参  获得当天
    const objDate = !y ? new Date() : new Date(y, parseInt(m) - 1, d);
    ...
    // 依据12节气修正干支月
    const gzM = calendar.toGanZhi((y - 1900) * 12 + m + 11);
    if (d >= firstNode) {
  ...
  getLunartoDay() {
    ...
    return `${today.gzYear}(${today.Animal})年 ${today.IMonthCn}${
      today.IDayCn
